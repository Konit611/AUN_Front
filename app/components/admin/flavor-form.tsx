...
export default function FlavorForm({ initial }: Props) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      if (isEdit && initial) {
        await apiPut<AdminFlavor>(`/admin/flavors/${initial.id}`, payload);
      } else {
        await apiPost<AdminFlavor>("/admin/flavors", payload);
      }
      router.push("/admin/sake/flavors");
      router.refresh();
    } catch (err) {
      setError(
  ...
  async function onDelete() {
    if (!initial) return;
    if (!confirm(`タグ「${initial.label}」を削除しますか？`)) return;
    setSubmitting(true);
    try {
      await apiDelete(`/admin/flavors/${initial.id}`);
      router.push("/admin/sake/flavors");
      router.refresh();
    } catch (err) {
      setError(
    ...
    <form onSubmit={onSubmit} className="flex flex-col gap-6 max-w-xl">
      ...
      <div className="flex items-center justify-between gap-4 pt-4 border-t border-border">
        ...
        <div className="flex gap-3">
          <button
            type="button"
            onClick={() => router.push("/admin/sake/flavors")}
            disabled={submitting}
