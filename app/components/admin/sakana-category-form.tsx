...
export default function SakanaCategoryForm({ initial }: Props) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      if (isEdit && initial) {
        await apiPut<AdminSakanaCategory>(
          `/admin/sakana-categories/${initial.id}`,
          payload,
        );
      } else {
        await apiPost<AdminSakanaCategory>("/admin/sakana-categories", payload);
      }
      router.push("/admin/sakana/categories");
      router.refresh();
    } catch (err) {
      setError(
  ...
  async function onDelete() {
    ...
    setSubmitting(true);
    try {
      await apiDelete(`/admin/sakana-categories/${initial.id}`);
      router.push("/admin/sakana/categories");
      router.refresh();
    } catch (err) {
      setError(
    ...
    <form onSubmit={onSubmit} className="flex flex-col gap-6 max-w-xl">
      ...
      <div className="flex items-center justify-between gap-4 pt-4 border-t border-border">
        ...
        <div className="flex gap-3">
          <button
            type="button"
            onClick={() => router.push("/admin/sakana/categories")}
            disabled={submitting}
