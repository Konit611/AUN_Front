...
export default function CategoryForm({ initial }: Props) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      if (isEdit && initial) {
        await apiPut<AdminPairingCategory>(
          `/admin/pairing-categories/${initial.id}`,
          payload
        );
      } else {
        await apiPost<AdminPairingCategory>("/admin/pairing-categories", payload);
      }
      router.push("/admin/pairing/categories");
      router.refresh();
    } catch (err) {
      setError(
  ...
  async function onDelete() {
    ...
    setSubmitting(true);
    try {
      await apiDelete(`/admin/pairing-categories/${initial.id}`);
      router.push("/admin/pairing/categories");
      router.refresh();
    } catch (err) {
      setError(
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col gap-6 max-w-xl">
      ...
      <Field label="ラベル (短い表示名)" required>
        <input
          type="text"
          value={label}
      ...
      <Field label="タイトル (ページヘッダー)" required>
        <input
          type="text"
          value={title}
      ...
      <div className="flex items-center justify-between gap-4 pt-4 border-t border-border">
        ...
        <div className="flex gap-3">
          <button
            type="button"
            onClick={() => router.push("/admin/pairing/categories")}
            disabled={submitting}
